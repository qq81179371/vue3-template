...
(function() {
    ...
    var getVideoHtml = function(data, videoTemplateCallback) {
        if (videoTemplateCallback) {
            ...
            return '<video style="width:' + data.width + 'px;height:' + data.height + 'px;" src="' + data.source + '" width="' + data.width + '" height="' + data.height + '"' + (data.poster ? ' poster="' + data.poster + '"' : '') + ' controls="controls">\n' + '</video>'
        }
    }
    ...
    var showDialog = function(editor) {
        ...
        var tabs = [
            generalTab,
            //embedTab //hxj，2020-12-22 16:25:42，移除内嵌
        ]
    ...
    var createPlaceholderNode = function(editor, node) {
        ...
        placeHolder.attr({
            ...
            'src': (name == 'video' && node.attr('poster') ? node.attr('poster') : global$8.transparentSrc),
            'data-mce-object': name,
            'class': 'mce-object mce-object-' + name
        })

        //placeHolder.attr({ 'src': 'https://p1.pstatp.com/list/240x240/tos-cn-i-0022/b3830cea6d404d72adeb039acabea7c6' });
    ...
    var createPreviewIframeNode = function(editor, node) {
        ...
        var previewNode = new global$7(name, 1)
            //Lc魔改开始部分
        if (name == 'video') {
            var vd = node.attributes.map
            var src = ''
            if (vd.src) {
                src = vd.src
            } else {
                var b = /<source([\s]+|[\s]+[^<>]+[\s]+)src=(\"([^<>"\']*)\"|\'([^<>"\']*)\')[^<>]*>/gi
                    //var s = vd.toLowerCase().match(b);
                var s1 = node.firstChild.value.toLowerCase().match(b)
                src = RegExp.$3
            }

            previewNode.attr({
                src: src,
                controls: vd.controls, //显示播放工具就取消这行的注释
    ...
    var placeHolderConverter = function(editor) {
        return function(nodes) {
            ...
            while (i--) {
                ...
                if (hasLiveEmbeds(editor) && global$8.ceFalse) {
    ...
    var setup$2 = function(editor) {
        ...
        editor.on('ObjectSelected', function(e) {
            var objectType = e.target.getAttribute('data-mce-object')
                //移除objectType === 'audio'允许音频组件被选中（hxj，2021-3-10 14:03:52）
